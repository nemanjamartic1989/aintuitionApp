<?php 
	...
	$languages = $_POST['languages'];
		...
		$sitesLanguagesUpdate = array($SiteSourceReputation => $languageReputation); // Creates an array by using the elements from one categories array and one points array.
		$sitesLanguagesUpdate = json_encode($sitesLanguagesUpdate); // Returns the JSON representation of a value.
		
		// SET UTF-8:
		$sitesLanguagesUpdate = preg_replace_callback('/\\\\u(\w{4})/', function ($matches) {
    		return html_entity_decode('&#x' . $matches[1] . ';', ENT_COMPAT, 'UTF-8');
		}, $sitesLanguagesUpdate);
			
	$querySourcesReputationUpdate = "UPDATE tdbSourcesReputation SET categories = '$categories', SiteSource = '$sitesLanguagesUpdate', createdBy = '$userID', createdDate = '$lastUpdate', userID = '$userID', lastUpdate = '$lastUpdate' WHERE SourcesReputationID = '$SourcesReputationID'";
	
	$resultSourcesReputationUpdate = mysql_query($querySourcesReputationUpdate);
		
		echo "<b class=\"red\" id=\"messageForUpdateDataOfSourcesReputation\">Data Updated!</b>";
		
	
?>

<fieldset id="ListOfSourcesReputationUpdate" style="margin: 50px 0 0 0;">
