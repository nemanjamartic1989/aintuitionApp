...
<fieldset><legend>List Of Product Category</legend>
	<label id="CategoryNameLabel">Product Category</label>
	
	<!-- Main list of Product Category -->
	<select id="listOfSubCategory2" name="listOfSubCategory" class="listOfSubCategory" onchange="listOfProductCategory(this.value)">
		<option value="">--</option>
		<?php 
		$nameCategory = $_POST['nameCategory'];
		$queryProduct = "SELECT * FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = 2";
		$rezProduct = mysql_query($queryProduct);
		$value = 0;
		
		if(mysql_num_rows($rezProduct) > 0){
			while($rowProduct = mysql_fetch_array($rezProduct)){
				$AIntuitionSubCategoryID = $rowProduct['AIntuitionSubCategoryID'];
				$AIntuitionCategoryID = $rowProduct['AIntuitionCategoryID'];
				$nameSubCategory = $rowProduct['nameSubCategory'];
		...
		$querySubProduct = "SELECT *, COUNT(AIntuitionSubCategoryID) AS subCount FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = '$AIntuitionCategoryID'";
		$rezSubProduct = mysql_query($querySubProduct);

		
		$rowSubProduct = mysql_fetch_array($rezSubProduct);
		$subCount = $rowSubProduct['subCount'];
	...
	<!-- Display Product Subcategory which choosed of certain Product Category -->
	<div id="displayHoldingBABUProduct" class="displayHoldingABU<?php echo $AIntuitionCategoryID;?>"></div>
