...
<fieldset><legend>List Of GRC Category</legend>
	
	<label id="CategoryNameLabel">GRC Category</label>
	
	<!-- Main list of Product Category -->
	<select id="listOfSubCategory7" name="listOfSubCategory" class="listOfSubCategory" onchange="listOfGRCCategory(this.value)">
		<option value="">--</option>
		<?php 
		$queryGRC = "SELECT * FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = 7";
		$rezGRC = mysql_query($queryGRC);
		$value = 0;
		
		if(mysql_num_rows($rezGRC) > 0){
			while($rowGRC = mysql_fetch_array($rezGRC)){
				$AIntuitionSubCategoryID = $rowGRC['AIntuitionSubCategoryID'];
				$AIntuitionCategoryID = $rowGRC['AIntuitionCategoryID'];
				$nameSubCategory = $rowGRC['nameSubCategory'];
		...
		$querySubGRC = "SELECT *, COUNT(AIntuitionSubCategoryID) AS subCount FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = '$AIntuitionCategoryID'";
		$rezSubGRC = mysql_query($querySubGRC);

		
		$rowSubGRC = mysql_fetch_array($rezSubGRC);
				$subCount = $rowSubGRC['subCount'];
	...
	<!-- Display GRC Subcategory which choosed of certain GRC Category -->

	<div id="displayHoldingBABUGRC" class="displayHoldingABU<?php echo $AIntuitionCategoryID;?>"></div>
