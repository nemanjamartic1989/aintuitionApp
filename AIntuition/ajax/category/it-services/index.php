...
<fieldset><legend>List Of IT Services Category</legend>
	
	<label id="CategoryNameLabel">IT Services Category</label>
	
	<!-- Main list of IT Services Category -->
	<select id="listOfSubCategory8" name="listOfSubCategory" class="listOfSubCategory" onchange="listOfITServicesCategory(this.value)">
		<option value="">--</option>
		<?php 
		$queryITServices = "SELECT * FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = 8";
		$rezITServices = mysql_query($queryITServices);
		$value = 0;
		
		if(mysql_num_rows($rezITServices) > 0){
			while($rowITServices = mysql_fetch_array($rezITServices)){
				$AIntuitionSubCategoryID = $rowITServices['AIntuitionSubCategoryID'];
				$AIntuitionCategoryID = $rowITServices['AIntuitionCategoryID'];
				$nameSubCategory = $rowITServices['nameSubCategory'];
		...
		$querySubITServices = "SELECT *, COUNT(AIntuitionSubCategoryID) AS subCount FROM `tdbAIntuitionSubCategory` WHERE AIntuitionCategoryID = '$AIntuitionCategoryID'";
		$rezSubITServices = mysql_query($querySubITServices);

		
		$rowSubITServices = mysql_fetch_array($rezSubITServices);
		$subCount = $rowSubITServices['subCount'];
	...
	<!-- Display IT Services Subcategory which choosed of certain IT services Category -->
	
	<div id="displayHoldingBABUITServices" class="displayHoldingABU<?php echo $AIntuitionCategoryID;?>"></div>
